...
/**
 * Класс Mr231_3Converter является частью системы, которая работает с типом станции "МР-231-3".
 * <p>
 * Класс Mr231_3Converter реализует интерфейс SearadarExchangeConverter и отвечает за
 * преобразование необработанных сообщений в список объектов SearadarStationMessage(TrackedTargetMessage,RadarSystemDataMessage).
 * <p>
 * Конвертер поддерживает два типа сообщений:
 * - TTM (формуляр цели) - объект типа TrackedTargetMessage;
 * - RSD (формуляр состояния НРЛС) - объект типа RadarSystemDataMessage.
 * <p>
 * Для каждого типа сообщения конвертер анализирует поля сообщения и создает соответствующий объект SearadarStationMessage.
 * Кроме того, преобразователь выполняет базовую проверку типа сообщения RSD и создает объект InvalidMessage, если обнаружены какие-либо ошибки.
 */
